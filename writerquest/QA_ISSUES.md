...
- [ ] **P0-1. trash가 Firestore 문서 1MB 한도 초과 가능**
...
- [ ] **P0-2. 스탯(style, imagination 등) 증가 로직 완전 미구현**
  - `StatPanel`에 5개 스탯 바가 표시되지만 어디에도 스탯을 올리는 코드가 없음
  - 게임 핵심 시스템이 껍데기만 존재
  - **Fix:** 집필 글자수/시간에 따라 스탯 증가 로직 구현

- [ ] **P0-3. 온보딩 완료 시 데이터 불일치 위험**
  - `completeOnboarding`에서 `_saveMeta()`와 `saveNovel()`을 별개 쓰기로 처리
  - `saveNovel` 실패 시 → Firestore에 `onboarded: true`지만 소설 없는 상태
  - **Fix:** Firestore `writeBatch`로 원자적 쓰기

- [ ] **P0-4. 저장 실패를 사용자에게 알리지 않음**
  - `saveNovel(uid, novel).catch((e) => console.error(...))`
  - Firestore 쓰기 실패 시 콘솔에만 출력, UI 피드백 전혀 없음
  - **Fix:** 저장 실패 시 Toast 경고 표시

- [ ] **P0-5. 오프라인 지원 없음**
  - Firestore 오프라인 퍼시스턴스 비활성화 상태
  - 네트워크 끊기면 저장 요청 모두 실패, 데이터 유실
  - **Fix:** `enableIndexedDbPersistence(db)` 또는 `enableMultiTabIndexedDbPersistence(db)` 활성화

---

## 🟠 P1 — 핵심 기능 오작동

- [ ] **P1-1. 인라인 에디터(CenterPanel)에서 스트릭이 쌓이지 않음**
...
- [ ] **P1-2. EditorPage의 저장 상태 표시 작동 안 함**
...
- [ ] **P2-1. 에러 바운더리 없음**
  ...
  - **Fix:** 최상위에 `<ErrorBoundary>` 추가

- [ ] **P2-2. BrowserRouter를 두 개 마운트 (App.jsx)**
  - `onboarded` 상태에 따라 두 개의 분리된 `<BrowserRouter>` 조건부 렌더링
  - 상태 전환 시 라우터 히스토리 초기화, 예상치 못한 동작 가능
  - **Fix:** 단일 `<BrowserRouter>`로 통합, 내부에서 조건부 라우팅

- [ ] **P2-3. beforeunload 리스너가 키입력마다 재등록됨 (CenterPanel)**
  - `content`가 deps에 있어 키입력마다 이벤트 리스너 추가/제거 반복
  - **Fix:** `contentRef` 패턴 사용 (EditorPage처럼)

- [ ] **P2-4. 소설 이모지 변경 불가**
  - 첫 소설 🌙, 이후 소설 📖로 고정
  - **Fix:** 소설 헤더 클릭 시 이모지 선택 UI 추가

- [ ] **P2-5. 퀘스트 표시 로직 불일치**
  - 완료된 일별 퀘스트는 계속 표시, 완료된 주별/1회성 퀘스트는 사라짐
  - **Fix:** 일관된 필터 로직 적용

- [ ] **P2-6. 로딩 타임아웃 후 재시도 방법 없음**
  - 10초 타임아웃 시 온보딩 화면으로 이동, 재시도 버튼 없음
  - **Fix:** 타임아웃/에러 시 "다시 시도" 버튼 표시

- [ ] **P2-7. SynopsisField Escape 시 편집 내용 소실**
  - 시놉시스 편집 중 Escape → 변경사항 미저장 닫힘
  - **Fix:** Escape 전 확인 또는 자동 저장

---

## 🔵 P3 — 마이너 버그 / 개선

- [ ] **P3-1. getRedirectResult와 onAuthStateChanged가 initUser 중복 호출 가능**
  - 두 경로가 거의 동시에 같은 uid로 `initUser` 호출 시 레이스 컨디션 가능

- [ ] **P3-2. firestoreSync.js 미사용 import 6개**
  - `addDoc`, `updateDoc`, `deleteDoc`, `writeBatch` import됐지만 미사용

- [ ] **P3-3. formatTime 함수 중복 정의**
  - `EditorPage.jsx`와 `CenterPanel.jsx`에 동일 함수 두 번 정의
  - **Fix:** `src/utils/formatTime.js`로 추출

- [ ] **P3-4. EditorStatusBar props sessionIdRef, initialWordCount 미사용**
  - Props 시그니처에 있지만 컴포넌트 내부에서 전혀 사용 안 함

- [ ] **P3-5. sessions 배열이 메모리에서 무한 증가**
  - `startSession`으로 추가만 되고 메모리에서 제거 안 됨
  - **Fix:** 완료된 세션은 Firestore 저장 후 메모리에서 제거

- [ ] **P3-6. PWA 아이콘이 SVG만 존재**
  - iOS는 SVG PWA 아이콘 미지원, 홈 화면 추가 시 기본 아이콘 표시
  - **Fix:** PNG 192×192, 512×512 추가

- [ ] **P3-7. 에너지 소모가 집필 여부와 무관**
  - 에디터 열어놓고 타이핑 안 해도 에너지 소모됨
  - **Fix:** 일정 시간 내 타이핑 없으면 에너지 소모 중단

---

## ⚪ P4 — 코드 품질

- [ ] **P4-1. initUser 얼리 리턴이 loading: true 상태 방치 가능**
  - `if (get().uid === uid) return` 시 loading이 true인 채로 스피너 무한 표시 가능

- [ ] **P4-2. EditorPage 챕터 탐색이 IIFE로 매 렌더마다 실행**
  - `const { novel, chapter } = (() => { ... })()` → `useMemo`로 교체 필요

- [ ] **P4-3. 접근성 — 기능 버튼에 aria-label 없음**
