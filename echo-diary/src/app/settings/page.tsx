...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {status && <p className="text-sm text-ink/55">{status}</p>}
      ...
      >
        <select
          ...
          className="w-full max-w-md rounded-xl border border-ink/15 bg-transparent px-3 py-2.5 text-sm outline-none focus:border-ink/40"
        >
          {MODELS.map((m) => (
      ...
      <Section title="주간 목표" description="한 주에 며칠 쓸지 정해두면 사이드바에 진행률이 표시됩니다.">
        <select
          ...
          className="w-full max-w-[10rem] rounded-xl border border-ink/15 bg-transparent px-3 py-2.5 text-sm outline-none focus:border-ink/40"
        >
          {[1, 2, 3, 4, 5, 6, 7].map((n) => (
      ...
      <Section title="계정">
        <div>
          <p className="text-sm text-ink/70">{user.displayName ?? "이름 없음"}</p>
          <p className="text-sm text-ink/45">{user.email}</p>
          {profile && (
            <p className="tabnum mt-2 text-xs text-ink/45">
