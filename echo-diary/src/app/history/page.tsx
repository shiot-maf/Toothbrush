...
function EntryRow({
  ...
  return (
    <Panel className="!p-0 overflow-hidden transition-colors hover:border-ink/25">
      <button
        onClick={onToggle}
        aria-expanded={open}
      ...
      {open && (
        <div className="reveal border-t border-ink/10 px-5 py-5">
          {entry.feedback && (
            <div className="mb-4 flex gap-1 rounded-full border border-ink/10 p-0.5 text-[10px] font-bold tracking-[0.14em] uppercase">
          ...
          <p className="content-text text-[16px] whitespace-pre-wrap">
          ...
          {entry.feedback?.overallComment && (
            <p className="mt-4 border-l-2 border-ink/15 pl-3 text-sm leading-relaxed text-ink/60">
              {entry.feedback.overallComment}
            </p>
          )}
          ...
          <div className="mt-5 flex flex-wrap items-center gap-4">
            <Link
              ...
              className="text-[11px] font-bold tracking-[0.16em] text-ink/55 uppercase underline-offset-4 hover:text-ink hover:underline"
            >
              {entry.feedback ? "교정 하나씩 보기 →" : "첨삭 받기 →"}
            </Link>
            {entry.feedback && (
              <span className="tabnum text-[11px] text-ink/35">
