...
export function MonthCalendar({ entries }: { entries: Entry[] }) {
  ...
  return (
    <section aria-label="작성 캘린더" className="space-y-3 border-t border-ink/10 pt-8">
      <div className="flex items-center justify-between">
        <h3 className="eyebrow">기록</h3>
        <div className="flex items-center gap-1">
          <NavButton label="이전 달" onClick={() => setMonthOffset((m) => m - 1)}>
            ‹
          </NavButton>
          <span className="tabnum min-w-[4.5rem] text-center text-[11px] text-ink/55">
            {label}
          </span>
          <NavButton
            label="다음 달"
        ...
        {WEEKDAYS.map((d, i) => (
          <span
            ...
            className={`text-[9px] font-semibold ${
              i === 0 ? "text-[var(--color-bad)]/50" : "text-ink/30"
            }`}
          >
            {d}
          </span>
      ...
      <p className="text-[11px] text-ink/45">
        이번 달 <span className="tabnum font-medium text-ink/70">{writtenThisMonth}일</span>{" "}
        기록했어요.
      </p>
    </section>
  )
    ...
    <span
      className={[
        ...
        corrected
          ...
          : written
            ? "border border-ink/45 font-medium text-ink"
            : isFuture
              ? "text-ink/15"
              : "text-ink/35",
        isToday && !written ? "ring-1 ring-ink/40" : "",
      ].join(" ")}
    >
      {day}
    </span>
...
function NavButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className="flex h-5 w-5 items-center justify-center rounded text-ink/40 transition-colors hover:bg-ink/8 hover:text-ink disabled:opacity-25 disabled:hover:bg-transparent"
    >
      {children}
    </button>
  )
