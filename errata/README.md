...
- **칭호** — 레벨 10·20·30·40·50·100에서 얻습니다.
- **알림** — 데이터 레이어가 화면을 직접 참조하지 않도록 `window` CustomEvent로
  던지고 `ToastHost`가 구독합니다.
...
`.github/workflows/errata-pages.yml`이 `errata/` 아래가 바뀔 때마다
...
```bash
NEXT_PUBLIC_BASE_PATH=/Toothbrush/errata npm run build   # Pages
npm run build                                                 # 루트 도메인
```

### Vercel

1. Vercel에서 이 리포를 import하고 **Root Directory를 `errata`로** 지정
